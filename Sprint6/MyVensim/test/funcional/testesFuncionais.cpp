...
public:
	...
	double equacao(){
		return 0.01 * ((this->getDestino())->getConteudo()) * (1 - ((this->getDestino())->getConteudo()) / 70);
	}
};

class MeuFluxo2 : public FluxHandle{

public:
	MeuFluxo2(string nome, Sistema* origem, Sistema* destino) : FluxHandle(nome, origem, destino){}


	double equacao(){
		return (this->getOrigem())->getConteudo()*0.01;
	}
};
...
void testeExponencialFuncional()
{
	...
	ModelHandle::deletaModelo("ModeloExponencial");
	delete m1;
}
...
void testeLogisticaFuncional()
{
	...
	Modelo* m2=ModelHandle::criarModelo();
	...
	ModelHandle::deletaModelo("Logistica");
	delete m2;
}
